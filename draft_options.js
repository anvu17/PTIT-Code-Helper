    ...
    if (modalCopyBtn) {
        modalCopyBtn.onclick = () => {
            ...
            navigator.clipboard.writeText(content).then(() => {
                showFeedback(modalCopyBtn);
            });
        };
    }
    modal.close.onclick = () => toggleClass(modal.self, 'pch-d-none', false);
    window.addEventListener('click', (e) => {
        if (e.target === modal.self) toggleClass(modal.self, 'pch-d-none', false);
    });
            ...
            if (problemIds.length === 0) {
                ...
                problemIds.forEach(problemId => {
                    ...
                    view.onclick = () => {
                        ...
                        toggleClass(modal.self, 'pch-d-none', true);
                    };
