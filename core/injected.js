    ...
    class PCHEditor {
        ...
        setupAutoSave() {
            ...
            if (this.settings.pch_autosave_mode === 'on_change') {
                ...
                const startInterval = () => {
                    if (!this.saveInterval) this.saveInterval = setInterval(save, interval);
                };
                const stopInterval = () => {
        ...
        submitCode(code) {
            if (!code || !code.trim()) {
                window.dispatchEvent(new CustomEvent('PCH_SHOW_TOAST', { detail: { message: 'Code trống!', type: 'error' } }));
                return;
            }
            ...
            if (!this.fileInputEl || !this.submitBtnEl) {
                window.dispatchEvent(new CustomEvent('PCH_SHOW_TOAST', { detail: { message: 'Không tìm thấy form nộp bài!', type: 'error' } }));
                return;
            }
