...
const DEFAULTS = {
    ...
    'pch_infinite_scroll': false,
    'pch_hide_chat': false,
    'pch_hide_lectures': false,
    'pch_hide_beta_banner': false,
    'pch_hide_contest_banner': false,
    'pch_hide_ac': false,
...
function toggleClass(element, className, hide) {
    if (hide) {
...
function fetchNotifications(onNotifFound) {
    chrome.runtime.sendMessage({ action: "fetchNotifications" }, (response) => {
        if (!response || response.error || !response.remote) return;
        onNotifFound(response.remote);
    });
}
